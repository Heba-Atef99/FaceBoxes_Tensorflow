...
def main():
    ...
    with graph.as_default():
        with tf.compat.v1.Session(graph=graph, config=config) as sess:
            tf.compat.v1.saved_model.loader.load(sess, [tf.compat.v1.saved_model.tag_constants.SERVING], ARGS.saved_model_folder)

            # output ops
            keep_nodes = ['boxes', 'scores', 'num_boxes']

            input_graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
            ...
            with tf.compat.v1.gfile.GFile(ARGS.output_pb, 'wb') as f:
