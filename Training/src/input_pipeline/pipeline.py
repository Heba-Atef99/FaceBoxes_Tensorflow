...
class Pipeline:
    ...
    def __init__(self, filenames, batch_size, image_size,
        ...
        def get_num_samples(filename):
            return sum(1 for _ in tf.compat.v1.io.tf_record_iterator(filename))
        ...
        self.iterator = tf.compat.v1.data.make_one_shot_iterator(dataset)
    ...
    def _parse_and_preprocess(self, example_proto):
        ...
        features = {
            ...
            'ymin': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
            'xmin': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
            'ymax': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
            'xmax': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
        }
        ...
        num_boxes = tf.cast(tf.shape(boxes)[0], tf.int32)
