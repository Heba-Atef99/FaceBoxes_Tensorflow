    ...
    def __call__(self, image_features, image_size):
        ...
        with tf.name_scope('anchor_generator'):
            ...
            for grid_size, box_spec in zip(feature_map_shape_list, self.box_specs_list):
                ...
                h, w = grid_size
                stride = (1.0/tf.cast(h, tf.float32), 1.0/tf.cast(w, tf.float32))
                offset = (0.5/tf.cast(h, tf.float32), 0.5/tf.cast(w, tf.float32))
...
def tile_anchors(
    ...
    # to [0, 1] range
    image_width, image_height = image_size
    height = unnormalized_height/tf.cast(image_height, tf.float32)
    width = unnormalized_width/tf.cast(image_width, tf.float32)
    ...
    y_translation = tf.cast(tf.range(grid_height), tf.float32) * anchor_stride[0]
    x_translation = tf.cast(tf.range(grid_width), tf.float32) * anchor_stride[1]
...
def generate_anchors_at_upper_left_corner(height, width, anchor_offset, n):
    ...
    center_ids = tf.cast(tf.range(n), tf.float32)
    # shape [n]
