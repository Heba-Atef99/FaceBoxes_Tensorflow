...
def _match(anchors, groundtruth_boxes, threshold=0.5):
    ...
    below_threshold = tf.cast(tf.greater(threshold, matched_vals), tf.int32)
...
def _create_targets(anchors, groundtruth_boxes, matches):
    ...
    matched_anchor_indices = tf.cast(matched_anchor_indices, tf.int32)
    unmatched_anchor_indices = tf.cast(unmatched_anchor_indices, tf.int32)
