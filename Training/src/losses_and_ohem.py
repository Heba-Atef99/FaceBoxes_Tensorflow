...
def _subsample_selection_to_desired_neg_pos_ratio(
    ...
    num_positives = tf.reduce_sum(tf.cast(positives_indicator, tf.int32), axis=0)
    max_negatives = tf.maximum(
        min_negatives_per_image,
        tf.cast(max_negatives_per_positive * tf.cast(num_positives, tf.float32), tf.int32)
    )

    top_k_negatives_indicator = tf.less_equal(
        tf.cumsum(tf.cast(negatives_indicator, tf.int32), axis=0),
        max_negatives
    )
