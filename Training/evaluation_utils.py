...
class Evaluator:
    ...
    def get_metric_ops(self, image_name, groundtruth, predictions):
        ...
        update_op = tf.numpy_function(update_op_func, tensors, [])

        def evaluate_func():
            self.evaluate()
            self.clear()
        evaluate_op = tf.numpy_function(evaluate_func, [], [])
        ...
        with tf.control_dependencies([evaluate_op]):
            ...
            eval_metric_ops = {
                'metrics/' + measure:
                (tf.numpy_function(get_value_func(measure), [], tf.float32), update_op)
...
def evaluate_detector(groundtruth_by_img, all_detections, iou_threshold=0.5):
    ...
    # each ground truth box is either TP(True Positive) or FN(False Negative)
    ...
    # acc, acc_op = tf.compat.v1.metrics.accuracy(gt_boxes, all_detections)
    return {
