...
def model_fn(features, labels, mode, params, config):
    ...
    with tf.name_scope('weight_decay'):
        ...
        regularization_loss = tf.compat.v1.losses.get_regularization_loss()

    # create localization and classification losses
    losses = detector.loss(labels, params)
    tf.compat.v1.losses.add_loss(params['localization_loss_weight'] * losses['localization_loss'])
    tf.compat.v1.losses.add_loss(params['classification_loss_weight'] * losses['classification_loss'])
    ...
    with tf.compat.v1.variable_scope('learning_rate'):
        ...
        learning_rate = tf.compat.v1.train.piecewise_constant(global_step, params['lr_boundaries'], params['lr_values'])
        tf.summary.scalar('learning_rate', learning_rate)

    update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops), tf.compat.v1.variable_scope('optimizer'):
        optimizer = tf.compat.v1.train.MomentumOptimizer(learning_rate, momentum=0.9, use_nesterov=True)
...
def add_weight_decay(weight_decay):
    ...
    for K in kernels:
        ...
        tf.compat.v1.add_to_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, x)
